...
static void Win32InitDSound(HWND Window, int32_t SamplesPerSecond, int32_t BufferSize)
{
    ...
    if(DSoundLibrary)
    {
        ...
        if(DirectSoundCreate && SUCCEEDED(DirectSoundCreate(0, &DirectSound, 0)))
        {
            ...
            if(SUCCEEDED(DirectSound->lpVtbl->CreateSoundBuffer(DirectSound, &BufferDescription, &GlobalSecondaryBuffer, 0)))
            {
...
static void Win32ClearSoundBuffer(struct Win32SoundOutput *SoundOutput)
{
    VOID  *Region1;
    DWORD Region1Size;
    VOID  *Region2;
...
static void Win32FillSoundBuffer(struct Win32SoundOutput *SoundOutput, DWORD ByteToLock, DWORD BytesToWrite, struct SoundOutputBuffer *SourceBuffer)
{
    VOID  *Region1;
    DWORD Region1Size;
    VOID  *Region2;
...
static void Win32ProcessXInputDigitalButton(DWORD XInputButtonState, struct GameButtonState *NewState, struct GameButtonState *OldState, DWORD ButtonBit)
{
...
{    
    ...
    if (RegisterClassA(&WindowClass))
    {
        ...
        if (Window)
        {
            ...
            int16_t *SoundMemory = (int16_t *)VirtualAlloc(0, SoundOutput.SecondaryBufferSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
            ...
            if(SoundMemory && GameMemory.PermanentStorage && GameMemory.TransientStorage)
            {
                ...
                while (Running)
                {
                    ...
                    {
                        struct GameControllerInput *OldController = &OldInput->Controllers[ControllerIndex];
                        struct GameControllerInput *NewController = &NewInput->Controllers[ControllerIndex];
                    ...
                    */
                    ...
                    if(SUCCEEDED(GlobalSecondaryBuffer->lpVtbl->GetCurrentPosition(GlobalSecondaryBuffer, &PlayCursor, &WriteCursor)))
                    {
                    ...
                    struct SoundOutputBuffer SoundBuffer = {0};
                    SoundBuffer.SamplesPerSecond = SoundOutput.SamplesPerSecond;
                    SoundBuffer.SampleCount = BytesToWrite / SoundOutput.BytesPerSample;
                    SoundBuffer.Samples = SoundMemory;
